...
- **向大型通讯组列表和外部用户发送电子邮件时，辅助功能检查器会发出提醒：** 已添自动提醒的功能，即通过邮件提醒，可在编写邮件发送给诸如大量受众和外部用户时，在辅助功能冲突的情况下，自动收到提示。请于"轻松使用"找到该设置。
...
- **在 PowerPoint 中插入 Flipgrid 视频:** PowerPoint 现支持在幻灯片中插入 Flipgrid 视频。
...
- 已修复在翻译已发布的批注时会导致错误消息“插入已翻译的文本失败”的问题。
