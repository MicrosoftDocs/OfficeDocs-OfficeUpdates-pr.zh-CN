...
- 修复了使用缩放窗口时应用程序意外关闭的问题。


- 此修复解决了之前尝试运行某些查询时产生“查询过于复杂”的错误信息的问题。


- 此更改修复了当启动缩放框（Shift + F2）编辑文本时，可能会导致 Access 崩溃的问题。
...
- 修正了一个问题，以便能够在应用程序不会出现任何故障的情况下将日期/时间扩展数据类型调用到代码中。


- 修正了一个问题，现在可以还原到最近更新的 Access 版本，并可使用 DAO/VBA 管理和编辑十进制数据类型。


- 我们修复了在尝试使用 ODBC DSN 生成器时的错误问题
...
- 修复了当你已从帐户注销时尝试创建数据连接时可能发生的问题。


- 修复了 PivotDateFilter API 中“Before”和“After”筛选条件相互的颠倒的错误。


- 解决了在试图将数据透视表插入图表工作表时 Excel 可能意外关闭的问题。
...
- 修复了当客户在共同创作时，会错误地收到关于新版本文件通知的问题。
...
- 修复了在使用某些Windows安全漏洞保护设置（SimExec、CallerCheck）时，Excel无法启动或意外崩溃的问题。


- 修复了某些情况下使用格式刷时 Excel 可能会崩溃的问题。


- 修复了用户无法修改数据透视表筛选器的问题，因为它被设置为在 Analysis Services 数据库中已不存在的值。


- 修复了Excel在冻结工作表首行后使用快速分析时可能会意外关闭的问题。
...
- 修复了在复制包含数据透视表的工作表后，在某些情况下会导致Excel意外关闭的问题。


- 修复了如果文件路径过长，重新打开文件后外部链接会停止工作的问题。


- 修复了在使用实时数据和多线程重新计算功能时可能导致的应用程序意外关闭的问题。


- 解决了如果关闭了源代码簿，则外部链接不会在填充时更新的问题。


- 修复了如果工作簿包含使用 IFNA() 的公式，则可能导致有关损坏的工作簿警告的问题。
...
- 我们修复了可选的连接体验会阻止Web插件加载的问题。  更多细节请看这里：https://developer.microsoft.com/en-us/office/blogs/outlook-add-ins-and-optional-connected-experiences/
...
- 我们修复了导致在组日历中搜索无法返回任何结果的问题。


- 修复了导致用户在与云端附件交互时偶尔会出现意外关闭的问题。


- 解决了CLP用户在将回复的地址从受保护的上下文切换到非受保护上下文时所遇到的问题。
...
- 解决了用户在选择 “仅下载邮件头” 选项时，从POP账户中删除4封或更多邮件时所遇到的问题。
...
- 解决了导致用户在第三方MAPI应用中所遇到的问题的问题。


- 解决了在 Windows 更新后开启保存在本地的 .msg 或 .oft 文件时，导致 Outlook 意外关闭的问题。


- 解决了导致Outlook在某些Windows版本上意外关闭的问题。
...
- 解决了导致用户看到Outlook不断提示他们运行收件箱修复工具的问题。


- 解决了导致用户在使用鼠标上的 "X "按钮时偶尔会出现问题的问题。


- 解决了用户在安全对话框中选择”save” 选项时无法将 OneDrive 附件从租户外部保存到本地计算机的问题。


- 我们修复了导致中文邮件在回复或转发时，标题会出现乱码的问题。
...
- 我们解决了导致某些情况下无法继续启用 "仅加密" 和 "不转发" 选项的问题，尽管策略已禁用这些情况。


- 我们修复了SmartLinks在保存到草稿时丢失格式的问题。


- 修复了替代策略时为用户提供自定义对齐文本方法的问题。


- 我们修复了另存为 OFT 文件时会导致汉字变成问号的问题。


- 解决了导致用户在检索角色信息时偶尔会出现意外关闭的问题。


- 这修复了用户在编辑收件人时偶尔会遇到应用程序关闭的问题。
...
- 我们修复了 Forms 内容加载项在插入后不显示，而需要到用户单击另一张幻灯片使其放映时才显示的问题。


- 安全性修补解决了在受保护视图中打开 PowerPoint 文件时禁用 IRM 保护的问题。


- 我们修正了 Slide.Shapes.AddMediaObject2 在使用传统视频格式（MPG-1,Mpeg-2）时会意外关闭的 VBA 问题。


- 我们修复了当用户将源路径更改为本地OneDrive文件夹时，链接的excel图表出现错误地更改为excel工作表的问题。


- 解决了当用户在文件中同时保留现代和遗留评论时的故障，从而触发对评论的升级。


- 我们修复了建议窗格可能会导致应用程序意外关闭的问题。


- 我们已修复了操作设置对话框中会导致 PowerPoint 应用意外关闭的问题。


- 我们修复了可能导致PowerPoint应用程序意外关闭的问题。
...
- 解决了在窗体视图中编辑前置/后续数据时，会触发额外的 ProjectBeforeTaskChangeevent 事件的问题。


- 解决了保存使用 Project 早期版本创建的项目时，Project 可能会意外关闭的问题。


- 修复了在启用保护实际工作的设置后，用户无法输入按时间分段的基准工作的问题。
...
- 修复了当以某种方式指定资源分布时，项目可能会在打开文件上意外终止的问题。
...
- 解决了导致用户在回复或撰写新邮件时遇到问题的问题。


- 解决了导致用户在使用鼠标上的 "X "按钮时偶尔会出现问题的问题。


- 修复了导致用户打开某些超大邮件时出现问题的问题。


- 我们修复了粘贴编号标题显示额外缩进的问题。
...
- 修复了一个评论扩展的bug，即评论回复的扩展名会与父评论相同。


- 我们修复了以下问题：如果你回复在扩展列表中具有未知扩展的父级评论，则回复将获得这些相同的扩展。


- 解决了可能导致应用程序在启动时意外关闭的问题。
...
- 修复了在旧的、非基于Web服务的共享窗格中，在共享窗格打开时关闭文档可能导致应用程序意外关闭的问题。


- 修正了可能导致应用程序在关闭办公文件时意外关闭的计时问题。
...
- 修复使用 GIF/动画模型 3D 时空闲的高 CPU 使用率
...
- 修复了一些损坏的 PowerPoint 文件无法正常打开（即使在执行文档修复操作后也是如此）的问题。
