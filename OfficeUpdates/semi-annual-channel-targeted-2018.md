...
这些发行说明提供了有关 2018 年 Office 365 专业增强版半年频道（定向）更新中所包含的新功能、安全更新和非安全更新的信息。
...
### <a name="access-feature-updates"></a>Access：功能更新
 - **利用新图表实现数据的可视化效果：** 从 11 个图表中选择一个图表并将其添加到窗体和报表，更好地实现数据的可视化效果，并做出明智的决策。[了解更多](https://support.office.com/article/1a463106-65d0-4dbb-9d66-4ecb737ea7f7)
...
### <a name="excel-non-security-updates"></a>Excel：非安全更新
-   修复了在通过图表的一组原始单元格更改图表的源数据时，Excel 可能崩溃的问题。
-   修复了即使在设置 FullCalcOnLoad 属性的情况下，打开时也不会进行重新计算的问题。  
-   修复了在日期单元格格式中使用日本和历日历时显示错误年份的问题。
-   将数据导入 Excel 数据模型时，传入的负零值将导致错误。此次修复将此类值导入为零。
-   修复了 Excel 数据透视表中的组合（或取消组合）操作有时可能会触发故障的问题。
-   修复了制作图表操作可能导致 Excel 崩溃的问题。
-   修复了对某些用户无意禁用 Power View 加载项的问题。
-   修复了永远不会清除文档恢复期间创建的临时自动恢复文件的问题。
-   修复了尝试对受保护的工作簿中的文本文件建立新连接时收到“工作簿处于受保护状态，无法更改”的错误消息的问题。
...
### <a name="outlook-feature-updates"></a>Outlook：功能更新
 ...
 - **从配置文件选取器中管理配置文件：** 如果在 Outlook 启动时使用配置文件选取器，你现在无需转到控制面板即可进行更改。 创建和删除配置文件、更改设置，所有操作都可通过配置文件选取器完成。
- **内置辅助功能：** 通过为图像添加描述性替换文字，让每个人都能访问你的邮件。
...
-   修复了对与会者更新的会议地点显示旧位置，而不是新位置的问题。
-   修复了用户在阅读窗格中预览附件时看到错误消息的问题。
-   修复了在用户撰写电子邮件期间，Outlook 将显示名称解析为电子邮件地址时发生故障的问题。
-   修复了一些用户没有收到租户管理员已启用的支持功能的问题。
...
- **避免意外编辑：** 将你的工作簿设置为以只读方式打开，以防止意外更改。 转到“文件”>“信息”>“保护工作簿”>“始终以只读方式打开”
- **辅助功能检查器改进：** 辅助功能检查器已更新了对国际标准与推荐的支持，以使你的演示文稿更易于访问。 [了解更多](https://support.office.com/article/a16f6de0-2f39-4a2b-8bd8-5ad801426c7f)

### <a name="powerpoint-non-security-updates"></a>PowerPoint：非安全更新程序
-   修复了表格显示不正常并带有粗边框的问题。
-   修复了更改 Shape.Visibile 属性时，可能会发生潜在崩溃的问题。
-   修复了无法合并合著文档中的更改的问题。
-   修复了包含 ActiveX 控件的文档会导致合著失败的问题。
-   修复了在形状中使用拼写更正导致 PowerPoint 崩溃的问题。
-   修复了从 SharePoint Online 打开文件时 PowerPoint 崩溃的问题。
-   修复了当自动保存处于打开状态时，恢复窗格错误显示的问题。
...
- **数据可视化工具审核模板：** 从 Excel 导入内容，并为财务事务、库存管理和更多内容创建审核图表。
- **起始图表：** 组织结构图、头脑风暴和 SDL 模板具有新的起始图表，让你能够快速使用起来。
...
### <a name="project-non-security-updates"></a>Project：非安全更新
-   修复了如果使用成本资源对任务进行拆分，则成本资源无法正确更新且成本丢失的问题。
-   修复了“日程表”视图中的“将现有任务添加到日程表”对话框仅显示第一个摘要任务中的任务的问题。
-   修复了将 Project Online 或 Project Server 中的主项目另存为 XML 可能会失败的问题。

### <a name="office-suite-non-security-updates"></a>Office 套件：非安全更新程序
-   修复了在某些应用场景中导致更新安装所用时间过长的 bug。 
-   修复了 SVG 测试失败的问题
-   修复了使用 System Center Configuration Manager 将更新部署到运行 Office 应用程序的客户端时，在 Office 应用程序正在运行的情况下重启设备后更新未应用的问题。


## <a name="version-1803-june-12"></a>版本 1803：6 月 12 日
*版本 1803（内部版本 9126.2227）*
...
### <a name="project-non-security-updates"></a>Project：非安全更新
-   修复了“日程表”视图“将现有任务添加到日程表”对话框仅显示第一个摘要任务中的任务的问题。
...
-   修复了 OneDrive for Business (Groove.exe) 在任务管理器中长时间使用 CPU 的一个 CPU 内核资源（例如，4 核 CPU 的 25%）的问题。
...
### <a name="word-non-security-updates"></a>Word：非安全更新
-   修复了如果运行 Windows 7 的计算机未安装 [客户体验和诊断遥测更新](https://support.microsoft.com/help/3080149/update-for-customer-experience-and-diagnostic-telemetry)，无法打开 Word 的问题。
-   修复了无法打印列表中的项目符号的问题。
...
### <a name="excel-non-security-updates"></a>Excel：非安全更新
-   修复了如果编辑语言是日语、中文或韩语，那么尝试在“开始”选项卡上选择新字体或进行编辑时，Excel 可能会冻结的问题。
-   修复了在最小化 Excel 后打开工作簿时，缺少滚动条的问题。
...
### <a name="outlook-non-security-updates"></a>Outlook：非安全更新
-   修复了在搜索范围为全部邮箱时，搜索失败且看到“找不到匹配”错误消息的问题。
-   修复了以下问题：如果在使用可访问事件观察程序 (AccEvent.exe) 进行监视时切换文件夹，导致 Outlook 崩溃。
...
-   **快速启动：** 自动生成大纲，帮助用户开始研究他们所选的主题。[了解更多](https://support.office.com/article/4784f273-0b2c-456c-9c89-24e5b977c224)
...
### <a name="powerpoint-non-security-updates"></a>PowerPoint：非安全更新程序
-   修复了删除文档属性和个人信息导致无法保存到 SharePoint 的问题。
-   修复了引用基于 Flash Player 的 YouTube 嵌入代码，导致打开新窗口来播放视频的问题。旧嵌入代码现已升级为，引用基于 HTML5 的 YouTube 视频，以便正确就地播放这些视频。
-   修复了保存支持同步的文件时，Office 无法对磁盘执行写入操作，但 Office 仍继续将文件上传到 OneDrive 的问题。修复此问题后，用户现在会看到错误消息，且上传不会继续执行。

### <a name="project-feature-updates"></a>Project：功能更新
-   **“任务板”视图：** 在“任务板”视图中，对卡片上的任务进行排序。在任务板上的各栏之间重新排序和移动卡片，就跟在敏捷项目中一样。
-   **敏捷项目：** 使用积压工作 (backlog)、任务板、冲刺 (sprint) 等管理敏捷项目。支持使用 Scrum 或看板方法。[了解更多](https://support.office.com/article/1b9b44d7-fd8e-4b3b-ab94-2b97deb9945b)  
-   **在规划器中管理任务：** 将项目任务关联到规划器，并为任务创建计划。将任务分解为子任务，添加团队，分配任务，并管理任务板上的工作。

### <a name="project-non-security-updates"></a>Project：非安全更新
-   修复了在会话中设置多个基线会将 MOD\_DATE 值设为相同的值的问题。
-   修复了在“​​保存以共享”会话中删除“实际工时”后，它仍显示在报表中的问题。
-   修复了进行日程安排时在德语版本中使用“星期”日期格式返回错误的问题。
-   修复了在日程安排 Web 部件中编辑完成日期时任务每天停留 8 小时而不随着时间推移的问题。
-   修复了在意外位置绘制“进度点形状”的问题。
-   修复了 VBA 代码从项目中丢失的问题。
-   修复了即使还有剩余工时，任务也仍显示为已完成的问题。
-   修复了在使用任务路径功能时项目挂起的问题。
-   修复了时间刻度不显示时间刻度标签的问题。
-   修复了可视化报表显示信息不完整或完全无法显示信息的问题。
-   修复了如果保存失败，则会损坏文件，并导致 Project 在打开时发生故障的问题。
-   修复了无法在“日程表和工作组规划器”视图中拖动任务的问题。
-   修复了工作组生成器中不显示资源可用性的问题。
-   修复了图形指示器未正确显示的问题。
-   修复了在按小时或按天的基础上调配时项目挂起的问题。
-   修复了从 SharePoint 文档库使用主项目/子项目的问题。
-   修复了在将作业添加到固定工期任务时，最终可能获得无名资源的问题。
-   修复了更改受保护的工时看到不正确的错误消息的问题。
-   修复了转到包含多个图像的报表时，Project 可能会发生故障的问题。
...
-   修复了创建 Skype for Business 会议期间，终端服务器上运行的 Outlook 冻结的问题。
-   将 EnableRestoreOAuthUsedKeyWhenUsingCachedWebTicket 的默认值更改为 TRUE。
-   修复了当会议处于全屏模式时，“更多选项”和“邀请更多人”按钮隐藏的问题。
...
-   修复了在 Outlook 中单击会议请求内的“加入 Skype 会议”链接时，用户看到“找不到此 Skype 会议”错误消息的问题。
...
-   修复了当 EnableExternalP2PFileTransfer 策略设置为 false 时，用户仍能在会议中附加文件的问题。
-   修复了对话历史记录中显示的是呼叫方，而不是被叫方的问题。如果使用 Active Directory 修改被叫方的工作电话号码，就会出现此问题。
-   修复了对于向手机号码拨出的传出 PSTN 呼叫，对话历史记录中的呼叫历史记录内缺少接听人信息的问题。
-   修复了从 Outlook 中的电子邮件启动 IM 时，IM 的主题中没有电子邮件的主题行的问题。
-   修复了当 IM 对话窗口贴靠到一侧时，对话出现双层堆叠的问题。
-   修复了在 VDIv2 环境中，VbSS 屏幕共享请求显示为基于 RDP 的请求的问题。
-   修复了当呼叫转移失败时，失败通知中列出的是呼叫方，而不是未接来电的接听人的问题。
-   修复了客户端升级重定向横幅中隐藏“开始使用 Teams”按钮的问题。
-   修复了 IM 窗口中的 DPI 缩放的问题。
-   修复了 Skype for Business 联系人卡片中不显示 LinkedIn 数据的问题。
-   用户现在可以代表智能寻线组停止接听电话。
-   如果 Skype for Business 或 Teams 中有通话正在进行中，且接听或启动了新电话，现在可以自动保持呼叫。
-   修复了用户在共享全屏后无法发送 IM 的问题。
-   修复了会议厅中的用户在被拒绝进入会议时收不到通知的问题。
-   修复了在通话期间，自动增益控制的增加不受控的问题。
-   修复了如果会议室资源邮箱已添加到会议邀请，用户无法在“会议选项”中选择演示者的问题。
-   修复了如果 AllowlPVideo 设置为 False，桌面共享按钮在对等视频呼叫期间灰显的问题。
-   修复了将“会议选项”设置更改为对使用“禁用 IM”创建的现有会议启用 IM 后，IM 仍处于禁用状态的问题。
-   修复了在聊天窗口中将鼠标悬停在“插入链接”按钮之上时，工具提示不显示，并且在选择此按钮后看不到辅助功能名称的问题。
...
-   **创建移动应用程序的线框：** 使用模板创建移动应用程序的线框。[了解更多](https://support.office.com/article/2d54dc55-f5c4-49a2-85da-d649eb7fc281)
...
-   **快速访问网站和组：** 使用“文件”菜单处理常用网站和组中存储的文档。
-   **Microsoft Translator：** 在 Word 中直接使用 Microsoft Translator 将字词、短语或整篇文档翻译成其他语言。[了解更多](https://support.office.com/article/24a987b3-03a1-4c17-8c1b-54495fca6b17)
-   **数字铅笔：** 使用我们新的铅笔纹理记录或勾画创意灵感。只需倾斜支持的数字铅笔即可绘制底纹。
-   **LinkedIn 功能设置：** 转到“文件”\>“选项”\>“常规”，以控制是否在 Office 应用程序中显示 LinkedIn 功能。[了解更多](https://support.office.com/article/dc81cc70-4d64-4755-9f1c-b9536e34d381)
...
### <a name="access-non-security-updates"></a>Access：非安全更新
-   修复了在使用多项目窗体时，调整鼠标滚轮的位置或滚动条缩略图不更改窗体中显示的项目的问题。
