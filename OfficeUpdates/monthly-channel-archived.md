...
- 修正了当来自缓存的 URL 和来自 OneDrive 的 URL 不匹配时，文件将被打开为不同步的问题。



[//]: # (请勿移除错误详细信息内容结尾)

## <a name="version-2011-december-08"></a>版本 2011：12 月 8 日
*版本 2011（内部版本 13426.20332）*
...
## <a name="version-2011-december-02"></a>版本 2011：12 月 2 日
*版本 2011（内部版本 13426.20308）*
...
- 修复了以下问题：如果项目文件在加载到特定部分时出现问题，则可能会打开特定项目。
...
## <a name="version-2011-november-30"></a>版本 2011：11 月 30 日
...
- **导出范围内的动态 GIF：** 导出到动态 GIF 时选择一系列幻灯片。
...
- **在 Microsoft Teams 中共享文件：** [了解更多](/MicrosoftTeams/sharing-files-in-teams)
...
- **指向幻灯片的链接：** 让同事参与幻灯片的创作，让其直接转到你需要帮助的幻灯片上。
...
- 解决了 Microsoft Access 中的一个问题，该问题可能会导致在运行更新查询或在 SQL 中使用 UPDATE 语句时出现&quot;查询已损坏&quot;错误。
...
- 如果在 2019 年使用巴西利亚时区，则定期会议和约会将显示在 2020 年的错误时间段内。 此更改涉及到在巴西利亚时区设置的客户端以及在该时区设置的会议和约会。 
...
- 解决了 ODT 和 GPO 更新截止时间设置中相对截止日期仅在第一次设置时起作用，而修复程序为后续更新启用相对截止日期的问题。
...
- **在 Office 365 专业增强版的现有安装上安装 Microsoft Teams：** 从 6 月底开始，我们将在更新这些安装时将 Microsoft Teams 包含在 Office 365 专业增强版（和 Office 365 商业版）的现有安装中。 将添加 Teams 的日期取决于所使用的更新频道。 有关详细信息，请参阅使用 Office 365 专业增强版部署 Microsoft Teams。 [了解更多](/deployoffice/teams-install)
...
### <a name="project"></a>项目
- 解决了用户在打开只读项目时可能会收到多条消息的问题。

### <a name="office-suite"></a>Office 套件

- 为了保护 Office 客户的安全，现在仅使用 SHA-2 算法对 Microsoft Office 更新进行签名。<br>
...
- 允许从具有多个网络适配器的 Windows 10 计算机将文件传输到旧版 Office Communicator 客户端。
...
- **隐私控制：** 对诊断数据和连接体验的全新、更新和改进控制。 [了解详细信息](/DeployOffice/privacy/overview-privacy-controls?toc=%2fdeployoffice%2ftoc.json)
...
- 修复了因第三方 SfB/Lync SDK 应用存在而导致 Skype for Business 在响应聊天通知时停止响应的问题。
- 修复了将特定剪贴板内容粘贴到聊天中时应用崩溃的问题。
- 修复了阻止显示其中一个呼叫代理接听的呼叫队列呼叫的“接受者”信息的问题。
- 修复了当 Teams 用户加入 Skype for Business 会议时隐藏呼叫图标的问题。
...
- **Microsoft Teams 安装：** 默认情况下，将为新安装的 Office 365 ProPlus 安装 Microsoft Teams。[了解详细信息](/DeployOffice/teams-install)
...
- **查看安全链接背后的 URL** 安全链接有助于保护你免受电子邮件中收到的恶意 URL 的攻击，但它们会隐藏原始 URL。 若要查看原始 URL，请将鼠标悬停在 URL 上。 需要高级威胁防护许可证。 [了解更多](https://products.office.com/exchange/advance-threat-protection)
...
### <a name="outlook-non-security-updates"></a>Outlook：非安全更新
 - 解决了一个问题，该问题导致了配置了多个 Exchange 帐户的某些用户的搜索结果列表中缺少“单击以查看更多...”链接。

### <a name="office-suite-non-security-updates"></a>Office 套件：非安全更新
 - 修复了在某些场景中导致更新安装所用时间过长的问题。
 
### <a name="lync-non-security-updates"></a>Lync：非安全更新
 - 修复了导致 IM 消息中的表情符号无法显示的问题。 
...
### <a name="outlook-feature-updates"></a>Outlook：功能更新
 ...
 - **日历共享简易化：** 共享日历变得更为简单，从 Outlook 桌面版共享的日历现在同样适用于 Outlook Mobile。[了解详细信息](https://support.office.com/article/353ed2c1-3ec5-449d-8c73-6931a0adab88)

### <a name="outlook-non-security-updates"></a>Outlook：非安全更新
- 将共享日历邀请中的字词从“打开此日历”更改为“接受”。
...
### <a name="word-feature-updates"></a>Word：功能更新
 - **在编辑时与合著者进行交流：** 无需退出 Word 即可与合著者进行交流，从而提高协作效率。[了解详细信息](https://support.office.com/article/1ecc6c7f-0b02-4baa-b9d9-c9d67023bedd)
...
### <a name="word-feature-updates"></a>Word：功能更新
-   **发现并修复相关校对问题：** 现在，可以在“编辑器”窗格中概览文档的校对问题，这样就能重点修复与自己最相关的问题了。
...
*版本 1712（内部版本 8827.2179）*

### <a name="excel-non-security-updates"></a>Excel：非安全更新
-   修复了最小化 Excel 时打开工作簿后滚动条缺失的问题。
...
### <a name="outlook-feature-updates"></a>Outlook：功能更新
-   **将 SVG 图标转换为形状：** 将所有 SVG 图片和图标都转换为 Office 形状，这样就可以更改它们的颜色、尺寸或纹理。
-   **Office 365 群组的改进：** 阅读和回复群组对话比以往更加简便，因为你可以双击群组消息在其自己的窗口中打开。
...
### <a name="visio-feature-updates"></a>Visio：功能更新
-   **使用 Excel 模板导出到 Visio 关系图：** 跳转到 Visio 流程图。在其中一个流程图 Excel 模板中输入数据并导出到 Visio，以自动创建关系图。需要安装 Visio Pro for Office 365。
...
*版本 1711（内部版本 8730.2165）*

### <a name="excel-non-security-updates"></a>Excel：非安全更新
-   修复了在编程创建数据透视表后，编程刷新导致 Excel 崩溃的问题。
-   修复了以编程方式调用 Workbook.Open() 可能导致 Excel 崩溃的问题。
